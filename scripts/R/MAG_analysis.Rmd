...
## Creating a text file for all species which express an exoPS
...
proxi_filtered <- proxi %>% mutate(MAG = str_sub(Target_label, end = -7)) %>% 
  mutate(Psiblast = str_replace(Psiblast, "alginate", "Alginate"),
  ...
  group_by(MAG) %>% summarise(exoPS = list(unique(Psiblast)))

# proxi_filtered$exoPS <- proxi_filtered$exoPS <- sapply(proxi_filtered$exoPS, function(lst) paste(lst, collapse = ", "))
...
proxi_filtered_phylum <- proxi_filtered %>% left_join(phylum %>% select(MAG, Taxonomy)) %>% select(Taxonomy, exoPS)

write_tsv(proxi_filtered, "/mnt/ahd/EPS_PIPELINE/data/raw/arb_MAG_w_exoPS.txt")
